function day_1()
    ...
    [t_list_FE,X_list_FE,h_avg_FE, num_evals_FE] = explicit_RK_fixed_step_integration(my_rate_func,tspan,V0,h_ref,Forward_Euler);
    ...
    [t_list_EM,X_list_EM,h_avg_EM, num_evals_EM] = explicit_RK_fixed_step_integration(my_rate_func,tspan,V0,h_ref,Explict_Midpoint);
    ...
    for n=1:length(h_ref_list)
        ...
        %Forward Euler
        [XB,~]=explicit_RK_step(my_rate_func,tspan(1),V0,h_ref,Forward_Euler);
        tr_error_list_FE(n)=norm(XB-V_list);
        %Explicit Midpoint
        [XB,~]=explicit_RK_step(my_rate_func,tspan(1),V0,h_ref,Explict_Midpoint);
    ...
    [p_FE,k_FE]=loglog_fit(h_ref_list,tr_error_list_FE,filter_parmas)
    [p_EM,k_EM]=loglog_fit(h_ref_list,tr_error_list_EM,filter_parmas)
    ...
    figure();
    loglog(h_avg_list_FE,tr_error_list_FE,'o', 'MarkerSize',5, 'Color',[1 0 0], 'DisplayName','Forward Euler Error')
    hold on;
